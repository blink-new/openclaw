...
## Get user info
```bash
blink connector exec tiktok /user/info/ GET '{"fields":"open_id,display_name,avatar_url,follower_count,following_count,likes_count,video_count"}'
```

## List my videos
```bash
blink connector exec tiktok /video/list/ POST '{"fields":"id,title,create_time,cover_image_url,share_url,view_count,like_count,comment_count,share_count","max_count":20}'
```

## Query specific videos
```bash
blink connector exec tiktok /video/query/ POST '{"filters":{"video_ids":["{video_id}"]},"fields":"id,title,view_count,like_count,comment_count,share_count,create_time"}'
```

## Get video comments
```bash
blink connector exec tiktok /research/video/comment/list/ POST '{"video_id":"{video_id}","max_count":20,"fields":"id,text,like_count,reply_count,create_time"}'
```
