...
```bash
blink connector exec jira /search GET '{"jql":"assignee=currentUser() AND status!=Done","maxResults":20}'
```

## Get a specific issue
```bash
blink connector exec jira /issue/{issueKey} GET
```

## Create an issue
```bash
blink connector exec jira /issue POST '{
  "fields": {
    "project": {"key": "PROJ"},
...
```bash
blink connector exec jira /issue/{issueKey}/transitions POST '{"transition":{"id":"{transitionId}"}}'
```

## Get available transitions for an issue
```bash
blink connector exec jira /issue/{issueKey}/transitions GET
```

## Add a comment
```bash
blink connector exec jira /issue/{issueKey}/comment POST '{"body":{"type":"doc","version":1,"content":[{"type":"paragraph","content":[{"type":"text","text":"Comment text here"}]}]}}'
```
...
## Get sprint issues
```bash
blink connector exec jira /search GET '{"jql":"sprint in openSprints()","maxResults":50}'
```
