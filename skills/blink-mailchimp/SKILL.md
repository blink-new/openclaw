...
```bash
blink connector exec mailchimp /lists GET '{"count":20}'
```

## Get audience details
```bash
blink connector exec mailchimp /lists/{list_id} GET
```

## Get subscribers in an audience
```bash
blink connector exec mailchimp /lists/{list_id}/members GET '{"count":20,"status":"subscribed"}'
```

## Add a subscriber
```bash
blink connector exec mailchimp /lists/{list_id}/members POST '{"email_address":"user@example.com","status":"subscribed","merge_fields":{"FNAME":"John","LNAME":"Doe"}}'
```

## Get campaigns
```bash
blink connector exec mailchimp /campaigns GET '{"count":10,"sort_field":"send_time","sort_dir":"DESC"}'
```

## Get campaign report
```bash
blink connector exec mailchimp /reports/{campaign_id} GET
```

## Send a campaign
```bash
blink connector exec mailchimp /campaigns/{campaign_id}/actions/send POST
```

## Search members
```bash
blink connector exec mailchimp /search-members GET '{"query":"john@example.com"}'
```
