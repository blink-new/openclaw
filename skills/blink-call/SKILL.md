...
using `blink ai call`. The agent on the call uses full AI capabilities — it can
handle questions, follow a script, collect information, and respond naturally.
...
```bash
blink ai call "+447911123456" "Confirm the appointment for Jane Doe tomorrow at 3pm at our London office."
```

## Make a call — specific voice
```bash
blink ai call "+14155551234" "You are a customer service agent following up on order #5521." --voice openai:nova
```

## Start a call without waiting (fire and forget)
```bash
...
## Command signatures
```
blink ai call <phone-number> <system-prompt> [--voice <voice>] [--max-duration <seconds>] [--no-wait]
blink ai call-status <call-id>
```
