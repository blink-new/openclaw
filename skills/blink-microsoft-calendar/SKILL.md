...
```bash
blink connector exec microsoft_calendar /me/events GET \
  '{"$top":"20","$select":"subject,start,end,location,attendees","$orderby":"start/dateTime"}'
```

## Create event
```bash
blink connector exec microsoft_calendar /me/events POST '{
  "subject": "Team Sync",
...
## Get all calendars
```bash
blink connector exec microsoft_calendar /me/calendars GET
```

## Check availability
```bash
blink connector exec microsoft_calendar /me/getSchedule POST '{
...
## Update event
```bash
blink connector exec microsoft_calendar /me/events/{id} PATCH '{
...
## Delete event
```bash
blink connector exec microsoft_calendar /me/events/{id} DELETE
```
