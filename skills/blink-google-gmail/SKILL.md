...
## List messages (inbox)
```bash
blink connector exec google_gmail /users/me/messages GET \
...
```bash
blink connector exec google_gmail /users/me/messages/MESSAGE_ID GET \
  '{"format": "full"}'
```

## Send an email
```bash
blink connector exec google_gmail /users/me/messages/send POST '{
...
```bash
blink connector exec google_gmail /users/me/messages/MESSAGE_ID/modify POST '{
...
## Move to trash
```bash
blink connector exec google_gmail /users/me/messages/MESSAGE_ID/trash POST '{}'
```

## List threads
```bash
blink connector exec google_gmail /users/me/threads GET \
  '{"labelIds": "INBOX", "maxResults": 10}'
```

## Get a thread
```bash
blink connector exec google_gmail /users/me/threads/THREAD_ID GET
```
